...
void vectorField::randomizeField(float scale){
	for (int i = 0; i < fieldSize; i++){
        ...
        float x = (float)(ofRandom(20,50)) * scale;
        float y = (float)(ofRandom(5,20)) * scale;
        field[i].set(x,y);
    }
}
...
void vectorField::draw(){
    ...
    for (int i = 0; i < fieldWidth; i+=5){
        for (int j = 0; j < fieldHeight; j+=5){
